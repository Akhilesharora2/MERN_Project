...
const storage = multer.diskStorage({
    destination: (req, File, callback) => {
        callback(null, '../driveroadtest/public/uploads')
    },
...
router.put('/imageUpdate/:code', upload.single("stmtImage"), async (req, res) => {
    try {
        ...
        const imgupd = await images.findOne({ code: req.body.code }, (err, image) => {   
            if(image.image !== ' '){
        fs.unlinkSync(`${__dirname}/../../driveroadtest/public/uploads/${image.image}`);
            }
...
router.put("/deleteImages/:code", async (req, res) => {
    try {
        ...
        console.log("${__dirname}/../../driveroadtest/public/uploads/${image.image}");
        const code = Number(req.params.code);
