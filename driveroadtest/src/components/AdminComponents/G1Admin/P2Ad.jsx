...
useEffect(() => {
  ...
  Axios.get(`${BASE_URL}/G1Data`).then((response) => {
    ...
    setNewelement(response.data.P2);
  });
  const type = 'P2';
  ...
  const handleChangeForNewStatement = (event) => {
    NS = document.getElementById("NS_P2").value;
    N1 = document.getElementById("N1_P2").value;
    N2 = document.getElementById("N2_P2").value;
    N3 = document.getElementById("N3_P2").value;
    N4 = document.getElementById("N4_P2").value;
    N5 = document.getElementById("N5_P2").value;
  ...
  const newStatement = (id) => {
    if (change) {
      ...
      setStmt(true);
      document.getElementById("NS_P2").value = '';
      document.getElementById("N1_P2").value = '';
      document.getElementById("N2_P2").value = '';
      document.getElementById("N3_P2").value = '';
      document.getElementById("N4_P2").value = '';
      document.getElementById("N5_P2").value = '';
    } else {
  ...
  function toggleNewStatement() {
    var stmt = document.getElementById('NewStatementP2');
    if (stmt) {
  ...
  const Delete = (id, key) => {
    ...
    // setNewelement(newFinal);
    // setChange(true);
    const rqst = Axios.delete(`${BASE_URL}/P2/deleteImageDoc?id=${imgId}`);
    const rqst2 = Axios.put(`${BASE_URL}/P2/imageUpdateMany/` + (key + 1));
  ...
  const updateImage = (code, event) => {
    if (filename2 !== " " && event.target.id === "imgUpd") {
      ...
      Axios.put(`${BASE_URL}/P2/imageUpdate/` + (event.target.name), formData2)
  ...
  const deleteImages = (code, event) => {
    Axios.put(`${BASE_URL}/P2/deleteImages/` + (Number(event.target.id)))
  ...
  const handleSubmit = (id) => {
    ...
    if (stmt) {
      ...
      if (filename === " ") {
        ...
        Axios.post(`${BASE_URL}/P2/imageUploadCode`, { code: newelement.length });
      } else {
        formData.append("stmtImage", filename);
        Axios.post(`${BASE_URL}/P2/imageUpload`, formData);
      }
    }
  }
  ...
  return (    
    ...
    <h6>Traffic Signs #2 (P2Ad)</h6>
      ...
      <form onSubmit={() => newStatement(Response._id)} method="PUT" id='NewStatementP2' encType='multipart/form-data'>
        <h6>Add a new Statement: {newelement.length + 1}</h6>
        <h6 className='stmt'>Statement {newelement.length + 1}: <input type="text" onChange={handleChangeForNewStatement} id='NS_P2' name="NS" placeholder="Enter a new statement..." /> </h6>
        <h6 className='stmt'>Option 1: <input type="text" onChange={handleChangeForNewStatement} id="N1_P2" name="N1" placeholder='Option A' /> </h6>
        <h6 className='stmt'>Option 2: <input type="text" onChange={handleChangeForNewStatement} id="N2_P2" name="N2" placeholder='Option B' /> </h6>
        <h6 className='stmt'>Option 3: <input type="text" onChange={handleChangeForNewStatement} id="N3_P2" name="N3" placeholder='Option C' /> </h6>
        <h6 className='stmt'>Option 4: <input type="text" onChange={handleChangeForNewStatement} id="N4_P2" name="N4" placeholder='Option D' /> </h6>
      ...
      <h6 className='mt-5 mb-4 text-center'>Final</h6> <hr />
      {
        newelement.map((value, key) => {
          ...
          return <form onSubmit={() => handleSubmit(Response._id)} method="PUT" key={key}>
            ...
            {
              (images.length) >= (key + 1) ?
                `/TrafficSigns/${images[key].image}` !== '/TrafficSigns/ ' ?
                  //true if image available
                  <>
                    <img className='StmtImage' src={`/TrafficSigns/${encodeURI(images[key].image)}`} alt={encodeURI(images[key].image)} />
                    <br></br>
