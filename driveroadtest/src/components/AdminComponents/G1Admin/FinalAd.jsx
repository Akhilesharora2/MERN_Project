...
const FinalAd = () => {
  ...
  useEffect(() => {
    ...
    Axios.get(`${BASE_URL}/G1Data`).then((response) => {
      ...
      setNewelement(response.data.Final);
    });
    const type = 'Final';
  ...
  const handleChangeForNewStatement = (event) => {
    NS = document.getElementById("NS_Final").value;
    N1 = document.getElementById("N1_Final").value;
    N2 = document.getElementById("N2_Final").value;
    N3 = document.getElementById("N3_Final").value;
    N4 = document.getElementById("N4_Final").value;
    N5 = document.getElementById("N5_Final").value;
  ...
  const newStatement = (id) => {
    if (change) {
      ...
      setStmt(true);
      document.getElementById("NS_Final").value = '';
      document.getElementById("N1_Final").value = '';
      document.getElementById("N2_Final").value = '';
      document.getElementById("N3_Final").value = '';
      document.getElementById("N4_Final").value = '';
      document.getElementById("N5_Final").value = '';
  ...
  function toggleNewStatement() {
    var stmt = document.getElementById('NewStatementFinal');
    if (stmt) {
  ...
  const Delete = (id, key) => {
    ...
    // setNewelement(newFinal);
    // setChange(true);
    const rqst = Axios.delete(`${BASE_URL}/FinalAd/deleteImageDoc?id=${imgId}`);
    const rqst2 = Axios.put(`${BASE_URL}/FinalAd/imageUpdateMany/` + (key + 1));
  ...
  const updateImage = (code, event) => {
    if (filename2 !== " " && event.target.id === "imgUpd") {
      ...
      Axios.put(`${BASE_URL}/FinalAd/imageUpdate/` + (event.target.name), formData2)
  ...
  const deleteImages = (code, event) => {
    Axios.put(`${BASE_URL}/FinalAd/deleteImages/` + (Number(event.target.id)))
  ...
  const handleSubmit = (id) => {
    ...
    if (stmt) {
      ...
      if (filename === " ") {
        ...
        Axios.post(`${BASE_URL}/FinalAd/imageUploadCode`, { code: newelement.length });
      } else {
        formData.append("stmtImage", filename);
        Axios.post(`${BASE_URL}/FinalAd/imageUpload`, formData);
      }
    }
  }
  ...
  return (
    <>
      ...
      <form onSubmit={() => newStatement(Response._id)} method="PUT" id='NewStatementFinal' encType='multipart/form-data'>
        <h6>Add a new Statement: {newelement.length + 1}</h6>
        <h6 className='stmt'>Statement {newelement.length + 1}: <input type="text" onChange={handleChangeForNewStatement} id='NS_Final' name="NS" placeholder="Enter a new statement..." /> </h6>
        <h6 className='stmt'>Option 1: <input type="text" onChange={handleChangeForNewStatement} id="N1_Final" name="N1" placeholder='Option A' /> </h6>
        <h6 className='stmt'>Option 2: <input type="text" onChange={handleChangeForNewStatement} id="N2_Final" name="N2" placeholder='Option B' /> </h6>
        <h6 className='stmt'>Option 3: <input type="text" onChange={handleChangeForNewStatement} id="N3_Final" name="N3" placeholder='Option C' /> </h6>
        <h6 className='stmt'>Option 4: <input type="text" onChange={handleChangeForNewStatement} id="N4_Final" name="N4" placeholder='Option D' /> </h6>
        <select 
            ...
            {/* {`/uploads/${images[key].image}` === " " ? console.log(`/uploads/${images[key].image}`) : console.log(`/uploads/${images[key].image}`)} */}
            {
              (images.length) >= (key + 1) ?
                `/uploads/${images[key].image}` !== '/uploads/ ' ?
                  ...
                  <>
                    <img className='StmtImage' src={`/uploads/${encodeURI(images[key].image)}`} alt={encodeURI(images[key].image)} />
                    <br></br>
