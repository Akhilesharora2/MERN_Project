...
useEffect(() => {
  ...
  Axios.get(`${BASE_URL}/G1Data`).then((response) => {
    ...
    setNewelement(response.data.Exam3);
  });
  const type = 'E3';
  ...
  const handleChangeForNewStatement = (event) => {
    NS = document.getElementById("NS_E3").value;
    N1 = document.getElementById("N1_E3").value;
    N2 = document.getElementById("N2_E3").value;
    N3 = document.getElementById("N3_E3").value;
    N4 = document.getElementById("N4_E3").value;
    N5 = document.getElementById("N5_E3").value;
  ...
  const newStatement = (id) => {
    if (change) {
      ...
      setStmt(true);
      document.getElementById("NS_E3").value = '';
      document.getElementById("N1_E3").value = '';
      document.getElementById("N2_E3").value = '';
      document.getElementById("N3_E3").value = '';
      document.getElementById("N4_E3").value = '';
      document.getElementById("N5_E3").value = '';
  ...
  function toggleNewStatement() {
    var stmt = document.getElementById('NewStatementE3');
    if (stmt) {
  ...
  const Delete = (id, key) => {
    ...
    // setNewelement(newFinal);
    // setChange(true);
    const rqst = Axios.delete(`${BASE_URL}/E3/deleteImageDoc?id=${imgId}`);
    const rqst2 = Axios.put(`${BASE_URL}/E3/imageUpdateMany/` + (key + 1));
  ...
  const updateImage = (code, event) => {
    if (filename2 !== " " && event.target.id === "imgUpd") {
      ...
      Axios.put(`${BASE_URL}/E3/imageUpdate/` + (event.target.name), formData2)
  ...
  const deleteImages = (code, event) => {
    Axios.put(`${BASE_URL}/E3/deleteImages/` + (Number(event.target.id)))
  ...
  const handleSubmit = (id) => {
    ...
    if (stmt) {
      ...
      if (filename === " ") {
        ...
        Axios.post(`${BASE_URL}/E3/imageUploadCode`, { code: newelement.length });
      } else {
        formData.append("stmtImage", filename);
        Axios.post(`${BASE_URL}/E3/imageUpload`, formData);
      }
    }
  }
  ...
  return (    
    ...
    <h6>Knowledge Test 1(P1Ad)</h6>
      ...
      <form onSubmit={() => newStatement(Response._id)} method="PUT" id='NewStatementE3' encType='multipart/form-data'>
        <h6>Add a new Statement: {newelement.length + 1}</h6>
        <h6 className='stmt'>Statement {newelement.length + 1}: <input type="text" onChange={handleChangeForNewStatement} id='NS_E3' name="NS" placeholder="Enter a new statement..." /> </h6>
        <h6 className='stmt'>Option 1: <input type="text" onChange={handleChangeForNewStatement} id="N1_E3" name="N1" placeholder='Option A' /> </h6>
        <h6 className='stmt'>Option 2: <input type="text" onChange={handleChangeForNewStatement} id="N2_E3" name="N2" placeholder='Option B' /> </h6>
        <h6 className='stmt'>Option 3: <input type="text" onChange={handleChangeForNewStatement} id="N3_E3" name="N3" placeholder='Option C' /> </h6>
        <h6 className='stmt'>Option 4: <input type="text" onChange={handleChangeForNewStatement} id="N4_E3" name="N4" placeholder='Option D' /> </h6>
        <select 
      ...
      <h6 className='mt-5 mb-4 text-center'>Final</h6> <hr />
      {
        newelement.map((value, key) => {
          ...
          return <form onSubmit={() => handleSubmit(Response._id)} method="PUT" key={key}>
            ...
            {
              (images.length) >= (key + 1) ?
                `/Exam3/${images[key].image}` !== '/Exam3/ ' ?
                  //true if image available
                  <>
                    <img className='StmtImage' src={`/Exam3/${encodeURI(images[key].image)}`} alt={encodeURI(images[key].image)} />
                    <br></br>
